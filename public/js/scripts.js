...
async function saveChanges() {
  ...
  document.querySelectorAll("#selectedItems tr").forEach(row => {
    ...
    updatedVHS.push({ id, donor, title, releaseDate, donatedDate });
  });

  try {
    const response = await fetch('/vhs/api/v1/update', { 
      method: 'POST', 
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updatedVHS)
    });

    const result = await response.json();
    alert("VHS records updated successfully!");
    location.reload();
  } catch (error) {
